...
public class AppDriver {
   ...
   private static List<Shape> loadShapesFromFile(String fileName) {
       ...
       try (BufferedReader br = new BufferedReader(new FileReader(fileName))) {
           // Read the first line; if it contains only digits, skip it.
           String firstLine = br.readLine();
           if (firstLine != null && firstLine.trim().matches("\\d+")) {
               System.out.println("Skipping shape count: " + firstLine);
           } else if (firstLine != null && !firstLine.trim().isEmpty()) {
               // If the first line isn't just digits, process it as a shape.
               shapes.add(parseShape(firstLine));
           }
   ...
   private static Comparator<Shape> getComparator(char compareType) {
       switch (compareType) {
           ...
           case 'a': return new BaseAreaComparator();  // 'a' for base area
