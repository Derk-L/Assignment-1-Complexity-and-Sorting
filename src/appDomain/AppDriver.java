...
public class AppDriver {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < size; i++) {
            // Print the first element, every 1000th element, and the last element.
    ...
    private static Shape parseShape(String line) {
        ...
        switch (type.toLowerCase()) {
            case "cone":
                return new Cone(height, base);
            case "cylinder":
                return new Cylinder(height, base);
            case "squareprism":
                return new SquarePrism(height, base);
            case "triangularprism":
                return new TriangularPrism(height, base);
            case "pentagonalprism":
                return new PentagonalPrism(height, base);
            case "octagonalprism":
                return new OctagonalPrism(height, base);
            case "pyramid":
    ...
    private static Comparator<Shape> getComparator(char compareType) {
        switch (compareType) {
            case 'h':
                return (s1, s2) -> Double.compare(s2.getHeight(), s1.getHeight());
            case 'b':
    ...
    private static void applySortingAlgorithm(List<Shape> shapes, char sortType, Comparator<Shape> comparator) {
        switch (sortType) {
            ...
            case 'm':
                ...
                break;
            case 'q':
                SortUtility.quickSort(shapes, 0, shapes.size() - 1, comparator);
                break;
            case 'h':
